...
def is_transaction_successful(drink_price, quarter, dime, nickle, pennie):
    """CHECK THE CUSTOMER ORDER WITH THE PRICE AND GIVE BACK CHANGE IF
    THERE IS AND WILL ALSO RETURN TRUE IF THE MONEY PAID IS SUFFICIENT BY THE CUSTOMER"""
...
def payment(drink_price):
    """WILL TAKE THE drink price AND TAKE INPUT FROM THE CUSTOMER TO FORWARD IT
     TO FUNCTION price_check(drink_price, quarter, dime, nickle, pennie)
     IT ALSO RETURNS THE price_check(drink_price, quarter, dime, nickle, pennie)
     """
    ...
    return is_transaction_successful (drink_price, quarters, dime, nickle, pennies)
